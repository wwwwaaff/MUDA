...
def LeNet_dropout(W_l1RE, W_l2RE, shape, dropout_net=0):
    model = Sequential() # 16 32 64 128    256 64 1  
    model.add(Conv2D(16, (4, 4), strides = 2, padding='same',
                     kernel_initializer=RandomNormal(mean=0.0, stddev=0.01),
...
def train(model_name, EPOCHS, dataset, noisestddev, pretrained=False, idx=0):
    ...
    if model_name.lower() == 'vgg':
        ...
        model = LeNet_dropout(W_l1RE, W_l2RE, x_train.shape[1:], dropout_net=noisestddev)
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        for idx in range(7,10): #[0, 0.005, 0.01, 0.02, 0.04]:
            args.stddev = 0.5 #sdev
            args.idx = idx
            print("************ Gaussian stddev ************************", args.stddev)
